...
* **表示内容**:
    ...
    * プロフィール画像、自己紹介
* **カスタマイズ**: 名刺のデザインをカスタマイズできる（プロフィール画像、背景画像アップロード）
    * **CSS編集機能**: 将来実装予定（MVP後）
* **データ同期**:
    * ユーザーが任意のタイミングで「今すぐ同期」できる (手動同期) ✅ 実装済み
    * 連携サービス（GitHub等）の最新情報を自動で名刺に反映する (自動同期): 将来実装予定（MVP後）
...
* **データ**:
    ...
    * 参加者リスト
    * イベントでの名刺交換履歴
* **注記**: MVP段階では実装せず、ユーザーフィードバックを元に設計を見直す予定
...
* **バッジの種類**:
    ...
    * **カスタム**: ユーザーが自由に作成
* **表示方法**:
    * PublicCard/PrivateCardにバッジIDリストとして含まれる ✅ 実装済み
    * 表示/非表示をユーザーが個別に制御可能 ✅ 実装済み
    * 認証済みバッジのCSS編集不可機能: 将来実装予定（CSS編集機能実装時）
* **認証**:
    * 運営が公式認証バッジを発行（verified: true）: 将来実装予定（MVP後）
    * 現在はモデレータによるバッジ付与機能のみ実装済み ✅
